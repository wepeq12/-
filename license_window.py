...
class LicenseDialog(QDialog):
    ...
    def _build_ui(self):
        ...
        logo.setStyleSheet("color: #3b82f6; font-size: 16px; font-weight: 900; padding-bottom: 16px;")
    ...
    def _make_plan_card(self, plan_key, plan):
        ...
        header = QLabel(f"{plan['emoji']} {plan['name']}")
        header.setStyleSheet(f"color: {plan['color']}; font-size: 14px; font-weight: 900;")
        layout.addWidget(header)

        price_lbl = QLabel(f"${plan['price_1m']}/חודש")
        price_lbl.setObjectName("price_lbl")
        price_lbl.setStyleSheet("color: #f8fafc; font-size: 13px; font-weight: 700;")
        layout.addWidget(price_lbl)

        for feat in plan["features"][:4]:
            f = QLabel(feat)
            f.setStyleSheet("font-size: 10px; color: #94a3b8;")
            layout.addWidget(f)

        if plan.get("popular"):
            pop = QLabel("⭐ הכי פופולרי")
    ...
    def _select_duration(self, dur_key):
        ...
        for plan_key, card in self._plan_cards.items():
            ...
            if lbl:
                months = DURATIONS[dur_key][1]
                lbl.setText(f"${p} / {DURATIONS[dur_key][0]}")
        self._update_price_label()
    ...
    def _on_activate_result(self, result: LicenseResult):
        ...
        if result.valid:
            self._activate_status.setText(
                f"🎉 רישיון תקף!\n{PLANS.get(result.plan, {}).get('name', result.plan)} | "
